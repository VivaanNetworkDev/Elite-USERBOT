...
from Elite.database.pmpermitdb import get_approved_users, pm_guard
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Elite.set_block_message(Elite.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Elite.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Elite.get_pm_settings()
    await Elite.allow_user(chat_id)
