...
DEVS = [8088623806, 6799720846]
from Elite.helper.PyroHelpers import get_ub_chats
from Elite.modules.basic.profile import extract_user, extract_user_and_reason
from Elite.database import gbandb as Elite
from Elite.database import gmutedb as Gmute
from Elite.modules.help import add_command_help
...
async def gban_user(client: Client, message: Message):
    ...
    if (await Elite.gban_info(user.id)):
...
async def ungban_user(client: Client, message: Message):
    ...
    try:
        if not (await Elite.gban_info(user.id)):
        ...
        for good_boi in ung_chats:
            try:
                ...
                er += 1
        await Elite.ungban_user(user.id)
        msg = (
            r"**\\#UnGbanned_User//**"
            f"\n\n**First Name:** [{user.first_name}](tg://user?id={user.id})"
            f"\n**User ID:** `{user.id}`"
...
if ok:
 ...
 async def globals_check(client: Client, message: Message):
    ...
    if not user_id:
        return
    if (await Elite.gban_info(user_id)):
        try:
